...
public class I04_palidromicPartitioning {
	/*
	 * --------------------
	...
	private static void findSubstring(String str, int i, List<String> ds) {		// i=partitioning index
		...
		for (int k=i; k<str.length(); ++k) {
			if (isPalindrome(str, i, k)) {										// if STR[index..k] is palindrome
				ds.add(str.substring(i, k+1));									// add STR[index..k] in a temp data structure
				findSubstring(str, k+1, ds);									// recursively find valid substring in STR[k+1...end]
